function chaleur1D_imp(cfl,opt)
...
if (opt==1)
    ...
    for nn=10:-1:1
        ...
        Datas=[x(1,:);u(:,nn2)'];
        fileID=fopen(['chaleur1D_EI_' num2str(cfl) '_' sprintf('%03i',11-nn) '.txt'],'w');
    ...
    if (nn2~=Nt2+1)
        nn2=Nt2+1;
        Datas=[x(1,:);u(:,nn2)'];
        fileID=fopen(['chaleur1D_EI_' num2str(cfl) '_' sprintf('%03i',10-nn) '.txt'],'w');
        fprintf(fileID,'%2.6f \t %2.6f \n',Datas);
        fclose(fileID);
    end
    nn2=1;
    Datas=[x(1,:);u(:,nn2)'];
    fileID=fopen(['chaleur1D_EI_' num2str(cfl) '_' sprintf('%03i',0) '.txt'],'w');
    fprintf(fileID,'%2.6f \t %2.6f \n',Datas);
    fclose(fileID);
end
